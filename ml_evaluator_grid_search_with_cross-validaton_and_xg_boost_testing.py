...
summary_data = 'resources/train/test/wso2apimanagerperformanceresults.csv'
summary_data_second = 'resources/test/test/wso2apimanagerperformanceresults.csv'
...
epsilion_grid = [0.0001, 0.0005, 0.001, 0, 0.01, 0.1, 0.5, 1, 2, 4, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5]
...
def evaluator(y_ev, y_pred_ev):
    confidence_rmse = np.sqrt(mx.mean_squared_error(y_true=y_ev, y_pred=y_pred_ev))
    confidence_mape = mean_absolute_percentage_error(y_true=y_ev, y_pred=y_pred_ev)
...
current_result_array_test = evaluator(y_ev=y_tes, y_pred_ev=y_prd_test)
...
data_splits_2 = np.array([], dtype='float64')
data_splits_2 = data_reader(csv_file=summary_data_second, total_row=n_rows, thousands_splitter=t_splitter,
                            csv_select_columns=csv_select_cols, x_column_numbers=x_select_cols,
                            y_column_number=target)

y_prd_test2 = svr_model.predict(data_splits_2[0])
print "test test real\n", array_print(data_splits_2[1])
print "\ntest test pred\n", array_print(y_prd_test2)
eval_2 = evaluator(y_ev=data_splits_2[1], y_pred_ev=y_prd_test2)
print "\nerror of two data test = ", eval_2[1]
...
y_prd_test3 = xgb_model.predict(data_splits_2[0])
print "test test real\n", array_print(data_splits_2[1])
print "\ntest test pred\n", array_print(y_prd_test3)
eval_3 = evaluator(y_ev=data_splits_2[1], y_pred_ev=y_prd_test3)
print "\nerror of two data test = ", eval_3[1]